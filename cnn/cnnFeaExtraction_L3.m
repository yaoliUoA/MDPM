function cnnFeaExtraction_L3(classId)
addpath('../');
init;
...
if ~exist(fullfile(conf.dataDir,conf.cnnDir_Local_L3,num2str(classId)),'dir')
    mkdir(fullfile(conf.dataDir,conf.cnnDir_Local_L3,num2str(classId)));
end
...
CROPPED_DIM = 227;
 for i = 1:length(imIndex)
     ...
     imCropAll = [im2colstep(im(:,:,1),[conf.patchSizeL3,conf.patchSizeL3],[conf.stepSize,conf.stepSize]);...
               im2colstep(im(:,:,2),[conf.patchSizeL3,conf.patchSizeL3],[conf.stepSize,conf.stepSize]);...
               im2colstep(im(:,:,3),[conf.patchSizeL3,conf.patchSizeL3],[conf.stepSize,conf.stepSize])];
     ...
     for j = 1:size(imCropAll,2)
        imCrop = im2uint8(reshape(imCropAll(:,j),[conf.patchSizeL3,conf.patchSizeL3,3])); 
